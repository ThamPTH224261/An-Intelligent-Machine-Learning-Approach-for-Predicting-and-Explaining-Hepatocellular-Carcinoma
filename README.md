# An-Intelligent-Machine-Learning-Approach-for-Predicting-and-Explaining-Hepatocellular-Carcinoma
This repository provides a complete workflow for analyzing and modeling the Merged Dataset, including feature selection, visualization, model training, evaluation, and explainability.

Files Overview

Merged_Dataset.xlsx — Raw dataset

Visualazation_Merged_HCC.ipynb — General exploratory data visualization.

Feature_selection.ipynb — Feature selection using statistical and ML-based methods.

Train_Grid_Search_Merged_Dataset.ipynb — Trains ML models with hyperparameter tuning via Grid Search.

TEST_Grid_Search_Merged_Dataset.ipynb — Evaluates trained models on the test set.

Hyperparameter.xlsx — Contains the optimal hyperparameter values selected for each model during grid search and cross-validation.

Feature_Visualization.ipynb — Visualizes selected features and compares model performance on the test set.

(LIME)_Merged_Dataset.ipynb — Generates local explanations and plots average feature importance from LIME.

(SHAPLEY)_Merged_Dataset.ipynb — Visualizes feature values using SHAP.
